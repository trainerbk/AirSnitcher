...
if [[ -z "${SCAN_DETAIL}" ]]; then
    ...
    # Prefer 2.4GHz for stable rapid reconnects on mesh networks.
    # freq_list restricted to a single frequency means wpa_supplicant only
    # sees one AP — no bssid= needed, no roaming, no --same-bss conflicts.
...
# No bssid= in config — freq_list restricted to one frequency is enough to
# land on the same AP. Adding bssid= conflicts with airsnitch's --same-bss.
