...
# No bssid= in either block: dual-interface mode lets wlan0_gtk connect to a
# different mesh node (avoids driver rejecting two auths to the same BSSID).
# Single-interface --same-bss mode also requires no bssid=.
